...
from app.models import TopixDailyBarRecord


def test_topix_daily_bar_record_alias_mapping() -> None:
    record = TopixDailyBarRecord.model_validate(
        {
            "Date": "2026-03-18",
            "O": 2718.52,
            "H": 2730.11,
...
def test_jquants_client_fetch_topix_daily_bars_handles_pagination() -> None:
    async def run() -> None:
        responses = [
            {
                "data": [
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            assert request.url.path == "/v2/indices/bars/daily/topix"
        ...
        try:
            records = await client.fetch_topix_daily_bars(from_date="2026-03-18", to_date="2026-03-19")
        finally:
